...
ComputeIKMod::ComputeIKMod(const std::string &name, Stage::pointer &&child, bool right)
   : WrapperBase(name, std::move(child))
{
    ...
    p.declare<geometry_msgs::PoseStamped>("target_pose_left");
    p.configureInitFrom(Stage::INTERFACE, {"target_pose_left"});

    p.declare<geometry_msgs::PoseStamped>("target_pose_right");
    p.configureInitFrom(Stage::INTERFACE, {"target_pose_right"});

    right_ = right;
}

void ComputeIKMod::setTimeout(double timeout){
	setProperty("timeout", timeout);
}

void ComputeIKMod::setEndEffector(const std::string &eef){
	setProperty("eef", eef);
}

void ComputeIKMod::setTargetPoseLeft(const geometry_msgs::PoseStamped &pose)
{
    setProperty("target_pose_left", pose);
}

void ComputeIKMod::setTargetPoseRight(const geometry_msgs::PoseStamped &pose)
{
    setProperty("target_pose_right", pose);
}


void ComputeIKMod::setTargetPoseLeft(const Eigen::Affine3d &pose, const std::string &link)
{
	geometry_msgs::PoseStamped pose_msg;
	pose_msg.header.frame_id = link;
	tf::poseEigenToMsg(pose, pose_msg.pose);
    setTargetPoseLeft(pose_msg);
}

void ComputeIKMod::setTargetPoseRight(const Eigen::Affine3d &pose, const std::string &link)
{
    geometry_msgs::PoseStamped pose_msg;
    pose_msg.header.frame_id = link;
    tf::poseEigenToMsg(pose, pose_msg.pose);
    setTargetPoseRight(pose_msg);
}


void ComputeIKMod::setMaxIKSolutions(uint32_t n){
	setProperty("max_ik_solutions", n);
}

void ComputeIKMod::setIgnoreCollisions(bool flag)
{
...
void ComputeIKMod::onNewSolution(const SolutionBase &s)
{
    ...
    std::string tp = right_ ? "target_pose_right" : "target_pose_left";
    std::cout << "computing ik for " << tp << std::endl;

    geometry_msgs::PoseStamped target_pose_msg = props.get<geometry_msgs::PoseStamped>(tp);
	...
	if (ik_solutions.empty() && storeFailures()) {  // failed to find any solution
        ...
        state.properties().set("target_pose_right", props.get("target_pose_right"));
        state.properties().set("target_pose_left", props.get("target_pose_left"));
